...
class ThreadClass(QObject):
    ...
    def countEmg(self,fix,max1,max2,orderQty,worktime,typeMaxCnt,gr): 
        """
        긴급오더: 대상
        ...
        if str(gr)  == '2':
            max2 -= orderQty
            max1 -= orderQty
            maxCnt -= orderQty * worktime
            fix = orderQty
            return(fix,max1,max2,maxCnt)
        elif str(gr) == '1':
            max1 -= orderQty
            maxCnt -= orderQty * worktime
            fix = orderQty
            return(fix,max1,maxCnt)
    
    def countNonemg(self,fix,max1,max2,smt,worktime,typeMaxCnt,gr): 
        """
        긴급오더: 비대상
            1, 2차 max, 최대 착공량에서 설비능력 반영수량을 빼준다.
                ...
                Return:
                    ...
                    max1: 1차 max(갱신 후)
                    max2: 2차 max(갱신 후)
                    countMax: 최대착공수량(갱신 후)
        """
        countMax = typeMaxCnt #외부 딕셔너리 value를 가져와서 함수 내부에서 계산하기 위한 목적
        if str(gr) == '1':
            if max1 > smt:
                if countMax >= smt * worktime:
                    fix = smt
                else:
                    fix = math.floor(countMax / worktime)           
            else:
                if countMax >= max1 * worktime:
                    fix = max1
                else:
                    fix = math.floor(countMax / worktime)
            max1 -= fix
            countMax -= fix * worktime
            return(fix,max1,countMax)
        elif str(gr) == '2':
            if max1 > smt:
                if countMax >= smt * worktime:
                    fix = smt
            ...
            max2 -= fix
            max1 -= fix
            countMax -= fix * worktime #외부 딕셔너리 계산할 경우 별도의 변수로 계산 필요할것
            return(fix,max1,max2,countMax) #fix : 설비능력 반영수량, max1 : 1차 max, module type : 최대 착공수량
    
    def countNonemg2(self,fix,smt,worktime,typeMaxCnt,gr): #비 긴급오더, 기종분류표 비대상 or NC모듈인 경우 중복 구분 처리
        """
        긴급오더: 비대상
            최대 착공량에서 설비능력 반영수량(설비능력 반영수량_잔여)을 빼준다.
                ...
                Return:
                    ...
                    countMax: 최대착공수량(갱신 후)
        """
        countMax = typeMaxCnt #외부 딕셔너리 value를 가져와서 함수 내부에서 계산하기 위한 목적   #딕셔너리 잘못 가져옴 수정 필요
        if str(gr) == '2':
            if countMax >= smt:
                fix = smt
            else:
            ...
            countMax -= fix
            return(fix,countMax)
    def countEmg2(self,fix,typeMaxCnt,orderQty): #alarm detail 설정할 것
        """
        긴급오더: 대상,
        ...
        countMax -= orderQty
        fix = orderQty
        if countMax - orderQty < 0:
            
            alarmx = 1
        return(countMax,fix,alarmx)
    #기타 함수 추가
    ...
    def run(self):
        ...
        try:
            ...
            if os.path.exists(NonSp_BLFilePath): #파일이 없으면 빈 스트링
                ...
                df_levelingNonSp_BL['Linkage Number'] = df_levelingNonSp_BL['Linkage Number'].astype(str)
            ...
            if os.path.exists(NonSp_TerminalFilePath): #파일이 없으면 빈 스트링
                ...
                df_levelingNonSp_Terminal['Linkage Number'] = df_levelingNonSp_Terminal['Linkage Number'].astype(str)
            ...
            df_levelingSp['Linkage Number'] = df_levelingSp['Linkage Number'].astype(str)
            # if self.isDebug:
            #     df_sosFile.to_excel(r'd:\\FAM3_Leveling-1\\verification\\flow2.xlsx')
            ...
            # if self.isDebug:
            #     df_sosFile.to_excel(r'd:\\FAM3_Leveling-1\\verification\\flow3.xlsx')
            ...
            # if self.isDebug:
            #     df_MergeLink.to_excel(r'd:\\FAM3_Leveling-1\\verification\\flow4.xlsx')
            ...
            if self.isDebug:
                df_joinSmt.to_excel(r'd:\\FAM3_Leveling-1\\verification\\df_SmtAssyInven.xlsx')
            
            # if self.isDebug:
            #     df_SmtAssyInven.to_excel(r'd:\\FAM3_Leveling-1\\verification\\flow5.xlsx')
            ...
            # if self.isDebug:
            #     df_pdbs.to_excel(r'd:\\FAM3_Leveling-1\\verification\\flow6.xlsx')
            ...
            if self.isDebug:
                df_addSmtAssy.to_excel(r'd:\\FAM3_Leveling-1\\verification\\flow6-1.xlsx')
            # df_temp.to_excel(r'd:\\FAM3_Leveling-1\\verification\\df_temp.xlsx')
            # df_addSmtAssy.to_excel(r'd:\\FAM3_Leveling-1\\test\\ta40_test.xlsx')
            # df_addSmtAssy = pd.read_excel(r'd:\\FAM3_Leveling-1\\ta40_test.xlsx')

            # if self.isDebug:
            #     df_addSmtAssy.to_excel(r'd:\\FAM3_Leveling-1\\verification\\test\\flow7-검증.xlsx')
            ...
            # if self.isDebug:
            #     df_addSmtAssy.to_excel(r'd:\\FAM3_Leveling-1\\verification\\test\\flow7.xlsx')    

            dict_minContCopy = dict_minContCnt.copy()
            
            #긴급오더 - 내림차순 정렬(긴급오더 대상이 뒤에 나오면 최소 착공량을 초과할수도 있기 때문)
            df_addSmtAssy = df_addSmtAssy.sort_values(by=['긴급오더'], ascending=[False])
            #평준화 적용
            ...
            for i in df_addSmtAssy.index:
                if df_addSmtAssy['긴급오더'][i] == '대상':
                    ...
                    if dict_minContCopy[df_addSmtAssy['대표모델'][i]][0] >= int(df_addSmtAssy['미착공수주잔'][i]):
                        ...
                        dict_minContCopy[df_addSmtAssy['대표모델'][i]][0] = 0
                #test
                if df_addSmtAssy['대표모델'][i] =='F3UM12-0N':
                    print(dict_minContCopy[df_addSmtAssy['대표모델'][i]][0])
                    print(df_addSmtAssy['평준화_적용_착공량'][i])
            ...
            df_addSmtAssy = df_addSmtAssy.reset_index(drop=True)
            if self.isDebug:
                df_addSmtAssy.to_excel(r'd:\\FAM3_Leveling-1\\verification\\flow7.xlsx')
           
            #긴급오더test용 data
            # df_addSmtAssy = pd.read_excel(r'd:\\FAM3_Leveling-1\\flow8.xlsx')
            #SMT 잔여수량 적용
            ...
            if self.isDebug:
                df_addSmtAssy.to_excel(r'd:\\FAM3_Leveling-1\\verification\\flow8.xlsx')
            ...
            #dict_ModuleMax = {'모듈':module_loading, '비모듈':nonmodule_loading, '비대상':999999}
            
            dict_capableCnt = {} #모델 : 공수
            dict_firstMaxCnt = {} #1차_MAX_그룹 : 1차_MAX
            dict_secondMaxCnt = {} #2차_MAX_그룹 : 2차_MAX
            dict_module = {} #모델 : 구분 
            dict_modelFirstGr = {} # 모델 : 1차_MAX_그룹
            dict_modelSecondGr = {} # 모델 : 2차_MAX_그룹
            dict_ModuleMax = {} # '모듈' : 최대 착공 수량, '비모듈' : 최대 착공 수량
            dict_ModuleMax = {'모듈':module_loading, '비모듈':nonmodule_loading} # '모듈' : 최대 착공 수량, '비모듈' : 최대 착공 수량  '비대상'이 필요한가?
                ...
                if df_addSmtAssy['긴급오더'][i] == '대상':
                    if df_addSmtAssy['MODEL'][i] in dict_module.keys(): #기종분류표에 model이 있는가
                        if df_addSmtAssy['MODEL'][i] in dict_modelSecondGr.keys(): #2차 max값 유무
                            df_addSmtAssy['설비능력 반영수량'][i],dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]],dict_secondMaxCnt[dict_modelSecondGr[df_addSmtAssy['MODEL'][i]]],dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]] = self.countEmg(df_addSmtAssy['설비능력 반영수량'][i],
                            ...
                            if dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]] > df_addSmtAssy['미착공수주잔'][i] * dict_capableCnt[df_addSmtAssy['MODEL'][i]]: #최대 착공량 > 미착공 수주량 * 공수
                                if dict_secondMaxCnt[dict_modelSecondGr[df_addSmtAssy['MODEL'][i]]] - df_addSmtAssy['미착공수주잔'][i] >= 0: #2차 max - 미착공 수주량 >= 0
                                    if dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]] <= 0: #1차 max > 0
                                        # dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]] > 0: #1차 max > 0
                                        # continue
                                         #1차 max
                                        df_alarmDetail = self.concatAlarmDetail(df_alarmDetail,
                                                                    alarmDetailNo,
                            ...
                            if df_addSmtAssy['MODEL'][i] in dict_modelFirstGr.keys(): #1차 max값 유무
                                df_addSmtAssy['설비능력 반영수량'][i],dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]],dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]] = self.countEmg(df_addSmtAssy['설비능력 반영수량'][i],
                                                                                                                                                                                                                                                    dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]],
                                                                                                                                                                                                                                                    0,
                                                                                                                                                                                                                                                    df_addSmtAssy['미착공수주잔'][i],
                                                                                                                                                                                                                                                    dict_capableCnt[df_addSmtAssy['MODEL'][i]],
                                ...
                                if dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]] - df_addSmtAssy['미착공수주잔'][i] * dict_capableCnt[df_addSmtAssy['MODEL'][i]] > 0: #최대 착공량 - 미착공 수주량 * 공수 
                                    if dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]] - df_addSmtAssy['미착공수주잔'][i] <= 0:
                                        # dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]] - df_addSmtAssy['미착공수주잔'][i] > 0: #1차 max - 미착공 수량 > 0
                                        # continue
                                        #분류2 - 1차 max 알람처리
                                        df_alarmDetail = self.concatAlarmDetail(df_alarmDetail,
                                                                    alarmDetailNo,
                                ...
                                dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]],df_addSmtAssy['설비능력 반영수량'][i],alarmx = self.countEmg2(df_addSmtAssy['설비능력 반영수량'][i],
                                                                                                                        dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]],
                                                                                                                        df_addSmtAssy['미착공수주잔'][i])
                                if alarmx == 1:
                                    df_alarmDetail = self.concatAlarmDetail(df_alarmDetail,
                        ...
                        if dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]] > 0: #최대 착공 수량 > 0
                            if df_addSmtAssy['MODEL'][i] in dict_modelSecondGr.keys(): #2차 max값이 있는가
                                if dict_secondMaxCnt[dict_modelSecondGr[df_addSmtAssy['MODEL'][i]]] > 0: #2차 max > 0?
                                    if dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]] > 0: #1차 max > 0?
                                        if dict_secondMaxCnt[dict_modelSecondGr[df_addSmtAssy['MODEL'][i]]] >= df_addSmtAssy['SMT반영_착공량'][i]: #2차 max >= smt 반영 착공량[i]
                                            ...
                                            df_addSmtAssy['설비능력 반영수량'][i],dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]],dict_secondMaxCnt[dict_modelSecondGr[df_addSmtAssy['MODEL'][i]]],dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]] = self.countNonemg(df_addSmtAssy['설비능력 반영수량'][i],
                                                                                                                                                                                                                                                                    dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]],
                                                                                                                                                                                                                                                                    dict_secondMaxCnt[dict_modelSecondGr[df_addSmtAssy['MODEL'][i]]],
                                ...
                                if df_addSmtAssy['MODEL'][i] in dict_modelFirstGr.keys(): #1차 max값이 있는가
                                    # print(dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]])
                                    df_addSmtAssy['설비능력 반영수량'][i],dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]],dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]] = self.countNonemg(df_addSmtAssy['설비능력 반영수량'][i],
                                                                                                                                                                                            dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]],
                                                                                                                                                                                            0,
                                                                                                                                                                                            df_addSmtAssy['SMT반영_착공량'][i],
                                                                                                                                                                                            dict_capableCnt[df_addSmtAssy['MODEL'][i]],
                                    ...
                                    df_addSmtAssy['설비능력 반영수량'][i],dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]] = self.countNonemg2(df_addSmtAssy['설비능력 반영수량'][i],
                                                                                                                                    df_addSmtAssy['SMT반영_착공량'][i], 
                                                                                                                                    dict_capableCnt[df_addSmtAssy['MODEL'][i]],
                                                                                                                                    dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]],
                                                                                                                                    2)
                    ...
                    if df_addSmtAssy['MODEL'][i] in dict_module.keys(): #기종분류표에 model이 있는가
                        if dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]] > 0: #최대 착공 수량 > 0
                            if df_addSmtAssy['MODEL'][i] in dict_modelSecondGr.keys(): #2차 max값이 있는가
                                if dict_secondMaxCnt[dict_modelSecondGr[df_addSmtAssy['MODEL'][i]]] > 0: #2차 max > 0?
                                    if dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]] > 0: #1차 max > 0?
                                        if dict_secondMaxCnt[dict_modelSecondGr[df_addSmtAssy['MODEL'][i]]] >= df_addSmtAssy['SMT반영_착공량_잔여'][i]: #2차 max >= smt 반영 착공량[i]
                                            # print(dict_secondMaxCnt)
                                            # print(dict_firstMaxCnt)
                                            df_addSmtAssy['설비능력 반영수량_잔여'][i],dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]],dict_secondMaxCnt[dict_modelSecondGr[df_addSmtAssy['MODEL'][i]]],dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]] = self.countNonemg(df_addSmtAssy['설비능력 반영수량_잔여'][i],
                                                                                                                                                                                                                                                                    dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]],
                                                                                                                                                                                                                                                                    dict_secondMaxCnt[dict_modelSecondGr[df_addSmtAssy['MODEL'][i]]],
                                ...
                                if df_addSmtAssy['MODEL'][i] in dict_modelFirstGr.keys(): #1차 max값이 있는가
                                    df_addSmtAssy['설비능력 반영수량_잔여'][i],dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]],dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]] = self.countNonemg(df_addSmtAssy['설비능력 반영수량_잔여'][i],
                                                                                                                                                                                            dict_firstMaxCnt[dict_modelFirstGr[df_addSmtAssy['MODEL'][i]]],
                                                                                                                                                                                            0,
                                                                                                                                                                                            df_addSmtAssy['SMT반영_착공량_잔여'][i],
                                                                                                                                                                                            dict_capableCnt[df_addSmtAssy['MODEL'][i]],
                                    ...
                                    df_addSmtAssy['설비능력 반영수량_잔여'][i],dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]] = self.countNonemg2(df_addSmtAssy['설비능력 반영수량_잔여'][i],
                                                                                                                                    df_addSmtAssy['SMT반영_착공량_잔여'][i], 
                                                                                                                                    dict_capableCnt[df_addSmtAssy['MODEL'][i]],
                                                                                                                                    dict_ModuleMax[df_addSmtAssy['모듈 구분'][i]],
                                                                                                                                    2)
            ...
            df_addSmtAssy = df_addSmtAssy.reset_index(drop=True)
            if self.isDebug:
                df_addSmtAssy.to_excel(r'd:\\FAM3_Leveling-1\\verification\\flow10.xlsx')
